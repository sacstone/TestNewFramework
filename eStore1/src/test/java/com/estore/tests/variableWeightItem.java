...
public class variableWeightItem extends TestBase {
	...
	private float item1SubTotal;
	private float item2SubTotal;
	...
	public void variableWeight() throws ParseException, InterruptedException {
		...
		shp.clickCategory("Browse All Catalog A").delivery();
		...
		float item2Price = StringHandling.extractFloat(sp.quantityInPopup().getText());
	...
	//	itemSubTotal = Math.round(item1SubTotal + item2SubTotal);
	//	itemSubTotal = itemSubTotal.
		
	//	System.out.println("From Multiplication: " + itemSubTotal);
		...
		/***
		 * Weblist to loop through address
		 * 
		 * //List<WebElement> s1 =
		 * ckthree.storePickUpDateTime1().findElements(By.xpath("//p[contains(text(),
		 * 'Time:')]")); List<WebElement> s1 =
		 * ckthree.storePickUpDateTime1().findElements(By.tagName("p"));
		 * 
		 * 
		 * 
