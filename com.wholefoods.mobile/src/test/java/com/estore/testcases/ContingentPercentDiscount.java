...
public class ContingentPercentDiscount extends TestBase {
	...
	@Test(description = "Test case for Contingent Percent Discount With Coupon")

	public void contingentPercentDiscount()
			throws ParseException, InterruptedException, MalformedURLException, IOException {

		StoreHomePage shp = new StoreHomePage(driver);

		//shp.browseCategory("Browse All Catalog D");
		...
		Thread.sleep(2000);
		//shp.ca
		shp.clickCategory("Catalog D").selectProduct("QTY-1");
...
/*
	//	Assert.assertEquals(StringHandling.extractInt(c.itemQuantity()), quantityOnDetailsPage, "Quantity is NOT same");
		...
		c.setDiscountCode().sendKeys("Contingent");
		c.applyDiscount().click();

		Cart c1 = new Cart(driver);

	//	Assert.assertEquals("$35.94", c1.lineItemSubTotal(), "Item Subtotal is not correct");
	//	Assert.assertEquals((float) 3.59, c1.discountAmount(), "Discount is not correct");
	//	Assert.assertEquals((float) 32.35, c1.subTotal(), "Suntotal is not correct");
...
*/
		...
		cktwo.shippingFname("Josh");
		cktwo.shippingLname("Smith");
	...
	//	driver.switchTo().frame(driver.findElement(By.id("vantiv-payframe")));
		 ...
		 * ckthree.storeHome();
		 * 
		 * StoreHomePage shp1 = new StoreHomePage(driver); shp1.shoppingCart();
		 * 
		 * Cart c2 = new Cart(driver);
		 * 
		 * c2.remove();
		 * 
		 * Cart c3 = new Cart(driver); Thread.sleep(3000);
		 * 
